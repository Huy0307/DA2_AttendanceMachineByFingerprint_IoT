...
int currentPage = 0;
String uid;
const char *ssid = "Galaxy A50s";
const char *password = "datdoan1301";
...
String childPath[3] = { "/DELETE", "/ENROLL" };
  ...
  for (size_t i = 0; i < numChild; i++) {
    if (stream.get(childPath[i])) {
      ...
      if (stream.dataPath == "/ENROLL/ID") {
        ...
        data.id_enr = stream.value.toInt();
      ...
      if (stream.dataPath == "/DELETE/ID") {
        Serial.println("Connection!!");
        data.id_del = stream.value.toInt();
...
void loop() {
  ...
  if (data.tempC != 0) {
    if (Firebase.ready()) {
      ...
      Serial.printf("Set Status... %s\n", Firebase.RTDB.setString(&fbdo2, "/Current data/Status", "0") ? "ok" : fbdo2.errorReason().c_str());
    }
    tft.fillScreen(TFT_BLACK);
    main_lcd();
    data.tempC = 0;
...
void getID_FB_ENR() {
  ...
  if (data.id_enr != 255 && !Name.equals("NONE") && !Image.equals("NONE")) {
    Serial.println(data.id_enr);
    Serial.println(Name);
    ...
    if (Firebase.ready()) {
      Firebase.RTDB.setInt(&fbdo3, F("/ENROLL AND DELETE/ENROLL/ID"), 255);
      Firebase.RTDB.setString(&fbdo3, F("/ENROLL AND DELETE/ENROLL/NAME"), "NONE");
    }
    data.id_enr = 255;
    Name = "NONE";
    Image = "NONE";
...
void getID_FB_DEL() {
  if (data.id_del != 255) {
    ...
    main = 1;
    if (Firebase.ready()) {
      Firebase.RTDB.setInt(&fbdo3, F("/ENROLL AND DELETE/DELETE/ID"), 255);
    }
  }
}
