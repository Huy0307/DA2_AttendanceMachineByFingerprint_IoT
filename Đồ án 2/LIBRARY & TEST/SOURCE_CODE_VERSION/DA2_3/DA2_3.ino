...
void getID_FB_ENR() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (Firebase.ready()) {
      Firebase.RTDB.getInt(&fbdo3, F("/ENROLL/ID"));
      Firebase.RTDB.getString(&fbdo4, F("/ENROLL/NAME"));
      data.id = fbdo3.to<int>();
      Name = fbdo4.to<String>();
      Serial.println(data.id);
      Serial.println(Name);
      Serial.printf("Get int... %s\n", Firebase.RTDB.getInt(&fbdo3, F("/ENROLL/ID")) ? String(fbdo3.to<int>()).c_str() : fbdo3.errorReason().c_str());
      Firebase.RTDB.setInt(&fbdo3, F("/ENROLL/ID"), 255);
      Firebase.RTDB.setString(&fbdo4, F("/ENROLL/NAME"), "NONE");
      if (data.id != 255 && !Name.equals("NONE")) {
        String fid = String(data.id);
...
void getID_FB_DEL() {
  ...
  if (currentMillis - previousMillis >= interval) {
    previousMillis = currentMillis;
    if (Firebase.ready()) {
      Firebase.RTDB.getInt(&fbdo3, F("/DELETE/ID"));
      data.id = fbdo3.to<int>();
      Serial.println(data.id);
      Serial.printf("Get int... %s\n", Firebase.RTDB.getInt(&fbdo3, F("/DELETE/ID")) ? String(fbdo3.to<int>()).c_str() : fbdo3.errorReason().c_str());
      Firebase.RTDB.setInt(&fbdo3, F("/DELETE/ID"), 255);
      if (data.id != 255) {
        tft.fillScreen(TFT_BLACK);
        Serial.println(data.id);
        deleteFingerprint(data.id);
