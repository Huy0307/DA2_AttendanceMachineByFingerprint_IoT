...
void convertToImageName(int id, char* result) {
  char imageName[10];  // Định nghĩa một mảng ký tự có độ dài đủ để chứa chuỗi "/id.jpg"

  itoa(id, imageName, 10);  // Chuyển đổi số nguyên thành chuỗi ký tự

  // Nếu bạn muốn thêm dấu '/' vào đầu chuỗi
  char finalName[12];  // Định nghĩa một mảng ký tự có độ dài đủ để chứa chuỗi "/id.jpg" và ký tự kết thúc chuỗi ('\0')
  finalName[0] = '/';
...
void loop() {
  ...
  // tft.fillScreen(random(0xFFFF));
  // drawSdJpeg("/sky1.jpg", 0, 0);  // This draws a jpeg pulled off the SD Card
  // delay(2000);

  
  // tft.fillScreen(random(0xFFFF));
  // drawSdJpeg("/sky2.jpg", 0, 0);  // This draws a jpeg pulled off the SD Card
  // delay(2000);

  
  // tft.fillScreen(random(0xFFFF));
  // drawSdJpeg("/sky3.jpg", 0, 0);  // This draws a jpeg pulled off the SD Card
